...
export const getUnifiedBookingsByUser = async (userId: string) => {
  const [
    ...
    supabase.from('auto_bookings').select('id, user_id, status, created_at, auto_id, start_date, pickup_location, dropoff_location').eq('user_id', userId),
  ...
  const allBookings = [
    ...
    ...(autos || []).map(b => ({ ...b, type: 'auto', title: 'Auto Booking', date: b.start_date })),
...
export const createAutoBookingRecord = async (data: any) => {
  return await supabase
    .from('auto_bookings')
    .insert(data)
    .select()
...
export const getAutoBookingsByUser = async (userId: string) => {
  return await supabase
    .from('auto_bookings')
    .select('*')
    .eq('user_id', userId)
